...
func (h *sentPacketHandler) ReceivedAck(ackFrame *wire.AckFrame, withPacketNumber protocol.PacketNumber, encLevel protocol.EncryptionLevel, rcvTime time.Time) error {
	...
	utils.DebugfFEC("Received ACK %d", h.largestAcked)
...
func (h *sentPacketHandler) updateLossDetectionAlarm(now time.Time) {
	...
	if !h.handshakeComplete {
		...
		// RTO
		time_ := h.computeRTOTimeout()
		h.alarm = now.Add(time_)
	}
}
...
func (h *sentPacketHandler) detectLostPackets(now time.Time) {
	...
	if len(lostPackets) > 0 {
		for _, p := range lostPackets {
			utils.DebugfFEC("Queue packet LOST PACKET %d", p.Value.PacketNumber)
...
func (h *sentPacketHandler) SendingAllowed() bool {
	...
	if (maxTrackedLimited){
		utils.DebugfFEC("Max Tracked Limited\n")
	}
...
func (h *sentPacketHandler) queueRTO(el *PacketElement) {
	...
	utils.DebugfFEC("Queue packet RTO %d, NOW: %s", packet.PacketNumber, time.Now().String())
...
func (h *sentPacketHandler) GetCWND() (protocol.ByteCount){
	return h.congestion.GetCongestionWindow()
}

func (h *sentPacketHandler) GetBytesFlight() (protocol.ByteCount){
